...
def fb_ls(directory):
    ...
    return " , ".join(list(data_val.keys()))


def fb_cat(directory):

    val=requests.get(datanode+directory+'.json').json()
    return list(val.values())
#print(cat('Project_Dataset_copy'))

def fb_getPartitionLoc(directory):
    val=requests.get(datanode+directory+'.json').json()
...
def fb_readPartitionLocations(args):
    k=int(args[1])
...
# print(fb_readPartitionLocations('Project_Dataset_copy',2))
